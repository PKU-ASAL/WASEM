...
from eunomia.arch.wasm.instructions.ControlInstructions import C_LIBRARY_FUNCS, SGX_STANDARD_FUNCS
...
class WasmSSAEmulatorEngine(EmulatorEngine):
    ...
    def __init__(self, isglobal, bytecode = None, namelist = None, Engine = None, entryFunc = None):
        if isglobal:
            self.cfg = WasmCFG(bytecode, namelist)
            self.ana = WasmModuleAnalyzer(self.cfg.module_bytecode, namelist)
        ...
        if not isglobal:
            ...
            self.remove_unrelated_funcs(entryFunc)
        ...
        if isglobal:
            ...
            for _, data_section_value in enumerate(self.ana.datas):
                if _ == 0:
                    ...
                    self.ro_data_section[(offset, offset + size)] = data
        else:
            self.exported_func_names = Engine.exported_func_names
            self.ro_data_section = Engine.ro_data_section
    ...
    def remove_unrelated_funcs(self, entry_func):
        """
        ...
        while stack:
            ...
            for item_func_callee in item_func_callees:
                if item_func_callee not in visited:
                    ...
                    if item_func_callee in SGX_STANDARD_FUNCS or item_func_callee in [i[1] for i in self.ana.imports_func]:
                        continue
        ...
        for i, item in enumerate(self.ana.globals):
            ...
            if op_type == 'i32':
                op_val = BitVecVal(item[1], 32) if is_exported else BitVec(
                    'global_' + str(i) + '_i32_' + str(datetime.timestamp(datetime.now()))[-5:], 32)
            elif op_type == 'i64':
                op_val = BitVecVal(item[1], 64) if is_exported else BitVec(
                    'global_' + str(i) + '_i64_' + str(datetime.timestamp(datetime.now()))[-5:], 64)
            else:
                raise UnsupportGlobalTypeError
            state.globals[i] = op_val
            ####wyp####
            if i == 0:
                state.shadow_globals[i] = shadow(False, True, None, None, None, 1)
            else:
                state.shadow_globals[i] = shadow(False, False)
    ...
    def init_state(self, func_name, param_str):
        ...
        if param_str != '':
            for i, local in enumerate(param_str.split(' ')):
                state.local_var[i] = getConcreteBitVec(
                    local, func_name + '_loc_' + str(i) + '_' + local + '_round' + str(state.round))

        state.current_func_name = func_name

        ###wyp###
        from eunomia.arch.wasm.dwarfParser import get_func_index_from_state,get_func_DIE
        func_ind = get_func_index_from_state(self.ana, state)
        func_offset = 0
        func_DIE = get_func_DIE(self.ana, func_ind, func_offset)
        para_num = 0
        ...
        assert para_num == len(param_str.split(' '))
        ...
        #####wyp######
        state.memory_manager = memory_manager(state, self.ana)

        return state

    def init_locals(self, state, func_name, param_str):
        if param_str != '':
            for i, local in enumerate(param_str.split(' ')):
                state.local_var[i] = getConcreteBitVec(
                    local, func_name + '_loc_' + str(i) + '_' + local + '_round' + str(state.round))
    ...
    def emulate_one_instruction(self, instr, state):
        ...
        print("symbolic memory", state.symbolic_memory)
        #print("data section", self.ro_data_section, state.memory_manager.data_section)
        ...
        if instr.group == 'Memory':
            ret_states = instr_obj.emulate(state, self.ro_data_section, self.ana)
