...
def write_result(state, exit=False):
    ...
    """
    # sgx mode does not need to write result
    return
...
def write_vulnerabilities(state, vtype):
    ...
    with open(file_name, 'w') as fp:
        ...
        state_result["Basic_Blocks"] = state.block_list

        state_result["vulnerability"] = vtype

        state_result["iteration round"] = state.round
