...
class BitwiseInstructions:
    ...
    def emulate(self, state):
        ...
        if shadow1.pointer:
            _shadow = shadow1
        elif shadow2.pointer:
            _shadow = shadow2
        if _shadow:
            assert not _shadow.stack_pointer
            state.shadow_stack.append(shadow(taint, True, _shadow.base, _shadow.base_taint, _shadow.size, _shadow.stack_pointer))
        else:
