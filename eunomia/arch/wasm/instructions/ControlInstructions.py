...
from eunomia.arch.wasm.lib.sgx import SGXStandardFunction
from eunomia.arch.wasm.lib.Import import ImportFunction
...
TERMINATED_FUNCS = {'__assert_fail', 'runtime.divideByZeroPanic', 'abort', '__assert'}
...
SGX_STANDARD_FUNCS = {'sgx_is_within_enclave','sgx_is_outside_enclave','malloc','calloc','free','sgx_ocalloc','sgx_ocfree','sgx_ocall','memset','memcpy_s','memcpy'}
...
class ControlInstructions:
    ...
    def restore_context(self, state):
        ...
        caller_func_name, cur_bb, stack, shadow_stack, local, shadow_local, require_return = state.context_stack.pop()
        ...
        if require_return:
            ...
            return_shadow = state.shadow_stack.pop()

        # step 2
        state.current_func_name = caller_func_name
        state.current_bb_name = cur_bb
        state.symbolic_stack = stack
        state.shadow_stack = shadow_stack
        state.local_var = local
        state.shadow_local = shadow_local


        # step 3
        if require_return:
            state.symbolic_stack.append(return_val)
            state.shadow_stack.append(return_shadow)

    def deal_with_call(self, state, f_offset, analyzer):
        ...
        readable_callee_func_name = readable_internal_func_name(
            ...
            callee_func_name)
        #print(readable_callee_func_name)
        ...
        elif readable_callee_func_name in SGX_STANDARD_FUNCS:
            func = SGXStandardFunction(
                readable_callee_func_name, state.current_func_name)
            states = log_in_out(
                readable_callee_func_name, "sgx")(
                func.emul)(
                state, param_str, return_str, analyzer)
    ...
    def emulate(self, state, analyzer):
        ...
        if self.instr_name == 'nop':
            ...
            op = state.symbolic_stack.pop()
            shadow = state.shadow_stack.pop()
            assert not shadow.taint and not shadow.pointer
            ...
            return self.deal_with_call(state, f_offset, analyzer)
        else:
